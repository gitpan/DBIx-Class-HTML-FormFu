...

{
    ...
    is( $fs->get_field('hidden_col')->render->{value}, 1 );
    is( $fs->get_field('text_col')->render->{value}, 'a');
    is( $fs->get_field('password_col')->render->{value}, undef);
    
    my $checkbox = $fs->get_field('checkbox_col')->render;
    ...
    my $select = $fs->get_field('select_col')->render;
    ...
    my @radio = map { $_->render } @{ $form->get_fields('radio_col') };
    ...
    my @rg_option = @{ $fs->get_field('radiogroup_col')->render->{options} };
    ...
    # column is inflated
    my $date = $fs->get_field('date_col')->render->{value};
